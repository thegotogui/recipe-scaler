...
function calculateAll() {
    ...
    if (firstRun) {
        ...
        // If this is the first time it's running, go grab values from local storage, if any. (The test for "if" checks to see if there is a value)
        if (localStorage.getItem("gWeight")) {
            dividedBy = localStorage.getItem("dividedBy");
            gWeight = localStorage.getItem("gWeight");
            gScale = localStorage.getItem("gScale");
            ozWeight = localStorage.getItem("ozWeight");
            ozScale = localStorage.getItem("ozScale");
            whichMeasure = localStorage.getItem("whichMeasure");

            getObj('dividedBy').value = dividedBy;
            getObj("inGrams").value = gWeight;
            getObj("scaleReadingG").value = gScale;
            getObj("inOunces").value = ozWeight;
            getObj("scaleReadingO").value = ozScale;
...
function blankIt(mode) {
    ...
    for (let indexRow = 1; indexRow <= 5; indexRow++) {
        for (let indexCol = 1; indexCol <= 6; indexCol++) {
            ...
            // document.getElementById("newDistOp" + indexRow + indexCol).style.display = none;
            if (mode > 1) {
...
function cakeulateAll(mode) {
    ...
    console.log(`recipeDiameter: ${recipeDiameter}`);
    console.log(`availDiameter: ${availDiameter}`);
    ...
    // let biggerPan = Math.max(recipeDiameter, availDiameter),
    //     smallerPan = Math.min(recipeDiameter, availDiameter)

    // const largest = Math.max(biggerRadius, smallerRadius);
